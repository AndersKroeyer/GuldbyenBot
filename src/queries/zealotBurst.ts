...
const getZealotEvents = async ({ code, fightId, zealotId }: getZealotEventsParams): Promise<DamageEvent[]> => {
    const reportArgs: ReportDataReportArgs = { code };
    const eventArgs: ReportEventsArgs = { fightIDs: [fightId], dataType: EventDataType.DamageDone, targetID: zealotId };
    const data = await graphClient.request<Query>(gql`
        query ($code: String, $fightIDs: [Int], $targetID: Int) {
            reportData {
                report(code: $code) {
                    events(fightIDs: $fightIDs, targetID: $targetID, dataType: DamageDone, targetInstanceID: 2, limit: 1000) {
